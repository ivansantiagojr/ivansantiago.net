...
Sim, Python pode ficar bem complexo, acredito que isso seja até uma barreira de entrada às vezes — falo isso como alguém que já sofreu com _ambientes virtuais_ e arquivos de dependências (obrigado `pyproject.toml`). E agora, com a complexidade que a tipagem do Python ganha a cada lançamento, imagino que essa barreira esteja ainda maior, principalmente porque está cada vez mais difícil encontrar código Python sem anotações de tipos. Eu mesmo sou um dos culpados por isso, e gosto de usar e estudar os tipos da linguagem, mas vejo que é inegável a complexidade que isso adiciona.
